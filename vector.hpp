...
namespace cov {
	...
	class vector final {
		...
		constexpr size_t compute_capacity(size_t cap)
		{
			if(cap%chunk_size==0)
				return cap;
			else
	...
	public:
		...
		vector(const vector& vec):m_capacity(vec.m_capacity),m_size(vec.m_size),m_data(m_allocator.allocate(vec.m_capacity))
		{
